...
namespace CVLeap
{
	internal class CalibrateStereoCamera
	{
		private const int ImageNum = 13;
		private const int MaxScale = 2;
		private static readonly CvSize BoardSize = new CvSize(9, 6);
		private static readonly int AllPoints = ImageNum * BoardSize.Width * BoardSize.Height;
		...
		public CalibrateStereoCamera()
		{
			...
			int[] goodImagelist;
			FindChessboardCorners(fileNames, out imagePointsLeft, out imagePointsRight, out pointCountLeft, out pointCountRight,
				out goodImagelist);
		...
		public void FindChessboardCorners(string[][] fileNames, out CvPoint2D32f[] imagePointsLeft,
			out CvPoint2D32f[] imagePointsRight, out int[] pointCountLeft, out int[] pointCountRight, out int[] goodImagelist)
		{
			...
			for (i = 0; i < fileNames.Length; i++)
			{
				for (j = 0; j < 2; j++)
				{
					...
					for (int scale = 1; scale <= MaxScale; scale++)
					{
						...
						if (timg != img)
							timg.Dispose();
						found = Cv.FindChessboardCorners(img, BoardSize, out corners, out corner_count,
							ChessboardFlag.AdaptiveThresh | ChessboardFlag.NormalizeImage);
						if (found)
						{
							if (scale > 1)
							{
					...
					Console.Write(".");
					if (found)
						Cv.FindCornerSubPix(img, corners, corner_count, new CvSize(11, 11), new CvSize(-1, -1),
							new CvTermCriteria(30, 0.01));
					if (j == 0)
					{
